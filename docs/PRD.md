...
| Due dates with time logic, push notifications, in-app notification banners, email, SMS, any outbound message system, streaks, recurring reminders | §1.4 and §3.2. The board makes no demands and sends nothing — there is no backend to send from. Clock-toggle reminders and the day's To-Do surface are the bounded exception, ruled **B104** (issue #169) — a tap, a glow, and a card that shows up where the person already looks; no times are stored and nothing pushes. The end-of-day SMS summary is removed, ruled **B107**; the 2x2 home screen widget is dropped, ruled **B111** — nothing pushes, nothing sits outside the app (the "Today's To Do History" rail is killed by the same ruling, **B111**). Recurring reminders are ruled out, **B113** — the shipped reminder is the complete feature; a completed reminder is complete, and re-tapping the clock is the only return path (the month view is deferred to issue #191, parked behind #170) — with one bounded exception, ruled **B138** (issue #295, the owner's ruling of record: https://github.com/AlastairZeved/TheBoards/issues/295#issuecomment-5826290712): the Recurring Reminders strip component is IN SCOPE — chips read title + next occurrence, page left / page right paging, an orange plus add control, and persistence of the reminder records; B113's clause is superseded for this component only, the firing/notification mechanics stay OUT of scope, and the clock-toggle reminder of B108/B109 remains the complete reminder mechanism |
