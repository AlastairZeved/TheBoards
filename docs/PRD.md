...
| Due dates with time logic, push notifications, in-app notification banners, email, SMS, any outbound message system, streaks | §1.4 and §3.2. The board makes no demands and sends nothing — there is no backend to send from. Clock-toggle reminders and the day's To-Do surface are the bounded exception, ruled **B104** (issue #169) — a tap, a glow, and a card that shows up where the person already looks; no times are stored and nothing pushes. The end-of-day SMS summary is removed, ruled **B107**; the 2x2 home screen widget is dropped, ruled **B111** — nothing pushes, nothing sits outside the app (the "Today's To Do History" rail is killed by the same ruling, **B111**) |
