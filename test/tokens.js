...
console.log('\n[10] Self-hosted type, drawn icon, shipped cache (UIUX §13, B36, B50, B60)');
{
  ...
  ok('CACHE is todo-boards-v66 — the B119 bump that retires the All tab on tablet (issue #212)',
    /const CACHE = 'todo-boards-v66';/.test(sw), (sw.match(/todo-boards-v\d+/) || [])[0]);
  ok('the build handshake ships: OWN_BUILD stamped v66, cache-busted sw.js check, two-strike self-heal, updateViaCache none',
    /const OWN_BUILD = 'v66';/.test(app) && /cache: 'reload'/.test(app) &&
