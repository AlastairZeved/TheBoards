...
const T = {
  ...
  reminder: '#e6c2c9',    // the Remind tab's pink identity fill, kept for the record (UIUX §2.6.2, B126/B132)
  reminderGlow: '#c6a400', // the reminder card-frame bloom — the yellow the owner ruled (UIUX §2.6.2, B132)
...
console.log('\n[1] The surface ladder — tokens declared, luminances reproduce (UIUX §2.2, B58)');
{
  const need = {
    ...
    '--reminder': T.reminder, '--reminder-glow': T.reminderGlow, '--carried': T.carried,
...
console.log('\n[5c] The note toolbar\'s four tab fills — one luminance rung (UIUX §2.6.4, B127)');
{
  ...
  ok("the Remind tab's active fill is --tab-remind — fill only, NO bloom (B132)",
    /\.note\.reminder\s+\.note-clock\s*{[^}]*background:\s*var\(--tab-remind\)/s.test(css));
...
console.log('\n[5c] The Remind tab pink — value, dark-ink contrast, clock placement (UIUX §2.6.2, B126)');
{
  ...
  // B132 moved the GLOW to the card frame in its own yellow — the tab fills,
  // never blooms, and no bloom reads --reminder anywhere.
  ...
  ok('the active Remind tab takes NO bloom — no box-shadow on .note-clock (B132)',
    !/\.note-clock\s*{[^}]*box-shadow/s.test(css));
...
console.log('\n[5d2] The reminder card glow — the yellow the owner ruled, on the card frame (UIUX §2.6.2, B132)');
{
  ok(`--reminder-glow is ${T.reminderGlow}`,
    (declared['--reminder-glow'] || '').toLowerCase() === T.reminderGlow.toLowerCase(),
    declared['--reminder-glow']);
  // The carried amber's hue held, one more rung down (Y 0.5297 → 0.3847, B132);
  // above the crossover, so the card's dark ink is correct through the frame.
  ok('--reminder-glow with --ink-dark = 7.98', r2(contrast(T.reminderGlow, T.inkDark)) === 7.98,
    String(r2(contrast(T.reminderGlow, T.inkDark))));
  ok('--reminder-glow stays above the ink crossover', lum(T.reminderGlow) > 0.1788,
    String(r4(lum(T.reminderGlow))));
  // The glow is the CARD's: border + bloom on the note frame, the carried
  // pattern (§4.8), never a second colour.
  ok('the reminder card frame carries the yellow bloom (.note.reminder .note-text)',
    /\.note\.reminder \.note-text\s*{[^}]*border-color:\s*var\(--reminder-glow\)[^}]*box-shadow:[^}]*var\(--reminder-glow\)/s.test(css));
  // App-level like --carried: not rebound per board type.
  for (const name of LADDER_NAMES.filter(n => n !== 'To-Do')) {
    const decl = propsIn(LADDER[name].sel);
    ok(`${name}: --reminder-glow is not rebound per board type`, !('--reminder-glow' in decl));
  }
}
...
console.log('\n[10] Self-hosted type, drawn icon, shipped cache (UIUX §13, B36, B50, B60)');
{
  ...
  ok('CACHE is zeved-boards-v98 — version bumped (shipped bytes changed, the #295 recurring-reminders strip lands in the #293 reserved top space)',
    /const CACHE = 'zeved-boards-v98';/.test(sw), (sw.match(/zeved-boards-v\d+/) || [])[0]);
  ok('the build handshake ships: OWN_BUILD stamped v98, cache-busted sw.js check, two-strike self-heal, updateViaCache none',
    /const OWN_BUILD = 'v98';/.test(app) && /cache: 'reload'/.test(app) &&
  ...
  ok('the tap swaps the week view to the tapped date\'s Sunday-first week; the shipped today+6 window stays the default (issue #191)',
    /function calWeekOf\(anchor\)/.test(app) &&
    /const days = calWeekAnchor \? calWeekOf\(calWeekAnchor\) : calWindow\(\);/.test(app) &&
    /anchor\.getDate\(\) - anchor\.getDay\(\)/.test(app));
  ...
  // twitter:image, og:url and canonical are consumed by scrapers, never fetched
  // by the page. Strip those four tags before the no-CDN sweep.
  const headMeta = /<meta (?:property="og:(?:url|image)"|name="twitter:image")[^>]*>|<link rel="canonical"[^>]*>/g;
  ok('no CDN URL in index.html', !external.test(html.replace(headMeta, '')));
