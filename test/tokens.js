...
console.log('\n[10] Self-hosted type, drawn icon, shipped cache (UIUX §13, B36, B50, B60)');
{
  ...
  ok('CACHE is todo-boards-v48 — the bump that ships B103 (issue #164: tablet mode is orientation-blind at >=744px)',
    /const CACHE = 'todo-boards-v48';/.test(sw), (sw.match(/todo-boards-v\d+/) || [])[0]);
  ok('the build handshake ships: OWN_BUILD stamped v48, cache-busted sw.js check, two-strike self-heal, updateViaCache none',
    /const OWN_BUILD = 'v48';/.test(app) && /cache: 'reload'/.test(app) &&
