...
console.log('\n[10] Self-hosted type, drawn icon, shipped cache (UIUX §13, B36, B50, B60)');
{
  ...
  ok('CACHE is zeved-boards-v92 — version bumped (shipped bytes changed, fold-vs-keyboard guard, issue #281)',
    /const CACHE = 'zeved-boards-v92';/.test(sw), (sw.match(/zeved-boards-v\d+/) || [])[0]);
  ok('the build handshake ships: OWN_BUILD stamped v92, cache-busted sw.js check, two-strike self-heal, updateViaCache none',
    /const OWN_BUILD = 'v92';/.test(app) && /cache: 'reload'/.test(app) &&
